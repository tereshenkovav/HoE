...
{  Исправлена моя ошибка, не позволяющая работать регистронезависимому режиму
под Unix. См. в тексте вызовы функций AnsiLowerCase, AnsiUpperCase.
Заменить на правильные, собственные функции, лишенные бага букв Ч и Я.
}
...
constructor TRegExpr.Create;
 begin
  ...
  {$ELSE}
  fInvertCase:=TRegExpr.InvertCaseFunction;
  {$ENDIF}
 ...
 begin
  ...
  {$ENDIF}
   begin
    ...
    {$ifdef fpc}{$ifdef unix}
    Result := REChar(MyAnsiUpperCase(Ch)[1]) ;
    {$endif}{$endif}
    if Result = Ch then begin 
      Result := REChar(AnsiLowerCase(Ch)[1]);
      {$ifdef fpc}{$ifdef unix}
      Result := REChar(MyAnsiLowerCase(Ch)[1]) ;
      {$endif}{$endif}
    end ;
   end;
