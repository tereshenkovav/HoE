...
type
  ...
  ['{1BB9C7BC-D1B3-474E-A195-9F74B18D597C}']
    procedure Render(const X, Y: Single; const Align: Integer; const S: AnsiString);
    procedure PrintF(const X, Y: Single; const Align: Integer;
      const Format: AnsiString; const Args: array of const);
    ...
    function GetSprite(const Chr: AnsiChar): IHGESprite;
    function GetHeight: Single;
    function GetStringWidth(const S: AnsiString;
...
type
  ...
  protected
    { IHGEFont }
    procedure Render(const X, Y: Single; const Algn: Integer; const S: AnsiString);
    procedure PrintF(const X, Y: Single; const Align: Integer;
      const Format: AnsiString; const Args: array of const);
    procedure PrintFB(const X, Y, W, H: Single; const Align: Integer;
      const Format: AnsiString; const Args: array of const);
  ...
  private
    ...
    // TAV addon - total height
    FTotalHeight:Integer ;


    function GetLine(const FromFile, Line: PAnsiChar): PAnsiChar;
  public
...
var
  Data: IResource;
  Size: Longword;
  Desc, PDesc, PBuf: PAnsiChar;
  LineBuf: array [0..255] of AnsiChar;
  S: AnsiString;
  Chr: AnsiChar;
...
function THGEFont.GetLine(const FromFile, Line: PAnsiChar): PAnsiChar;
var
  I: Integer;
begin
  I := 0;
...
function THGEFont.GetSprite(const Chr: AnsiChar): IHGESprite;
begin
  Result := FLetters[Ord(Chr)];
end;

function THGEFont.GetStringWidth(const S: AnsiString;
  const FirstLineOnly: Boolean = True): Single;
var
  I: Integer;
  LineW: Single;
...
function THGEFont.GetTotalHeight: Integer;
begin
  Result:=FTotalHeight ;
end;
